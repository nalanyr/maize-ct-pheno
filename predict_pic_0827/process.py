...
class Worker(QThread):
    ...
    def __init__(self, settings: Settings, unet_embryos=None, unet_grains=None):
        ...
        self.saving_executor = ThreadPoolExecutor(max_workers=self.settings.save_threads, thread_name_prefix='SavingThread')
    ...
    def _preprocess_worker(self, img_list, folder_path, output_queue):
        ...
        with ThreadPoolExecutor(max_workers=self.settings.preprocess_workers) as executor:
            ...
            for i in range(0, len(img_list), bs):
                ...
                if batch_tensors:
                    batch_stacked = torch.stack(batch_tensors)
                    if torch.cuda.is_available():
                        batch_stacked = batch_stacked.pin_memory()
                else:
                    batch_stacked = None
    ...
    def _predict_worker(self, input_queue, output_queue, folder_name):
        active_device = self.unet_embryos.device if self.unet_embryos else (self.unet_grains.device if self.unet_grains else None)
        while not self.stop_event.is_set():
            data = input_queue.get()
            if data is None:
                output_queue.put(None)
                break
            batch_tensor_cpu, batch_shapes, batch_names, batch_otsu_masks = data
            pr_embryos, pr_grains = None, None
            if batch_tensor_cpu is not None and active_device is not None:
                batch_tensor_gpu = batch_tensor_cpu.to(active_device, non_blocking=True)
                pr_embryos = self.unet_embryos._detect_from_tensor(batch_tensor_gpu) if self.unet_embryos else None
                pr_grains = self.unet_grains._detect_from_tensor(batch_tensor_gpu) if self.unet_grains else None
