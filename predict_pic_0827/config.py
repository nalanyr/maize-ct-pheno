    ...
    def log_task_snapshot(self, logger):
        ...
        if torch.cuda.is_available():
            try:
                ...
                gpu_info = f"AVAILABLE ({torch.cuda.get_device_name(0)} | {vram_gb:.1f} GB VRAM)"
            except Exception as e:
                try:
                    gpu_info = f"AVAILABLE ({torch.cuda.get_device_name(0)})"
                except Exception:
                    gpu_info = f"AVAILABLE (unknown device; {e})"
